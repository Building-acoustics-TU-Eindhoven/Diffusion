...
## Objectives
The software provides the energy density and sound pressure level over 3D space and time and most of the energy parameters such as Reverberation time (RT), Early Decay Time (EDT), Clarity, Definition and Centre time.
...
The stability criterion is the following (Eq.18 of Navarro et al. 2012):
```{math}
...
Additional equations are needed to describe the sound field at the boundaries. The discretized equations are based on the forward and backward three points formula (Necati book). 
...
The software calculates the Sound Density Level at each mesh grid position of the room with the following formula
```{math}
SDL = 10 log_10⁡(w_{i,j,k}^{n+1})
```
It also calculates the Sound Pressure Level at the receiver position rec as follows:
```{math}
SPL = 20 log_10⁡(((w_{i_r,j_r,k_r}^{n+1}ρc_{0}^2 ))/p_{ref}^2) 
```


12. Calculate Reverberation time and EDT: -> Description!!!!!!!!!!!!!!!!!!!
The Reverberation time is calculated using backward integration and in between -5 and -35 dB difference. 

13. Calculate Clarity, Definition and Centre Time  -> Description!!!!!!!!!!!!!!!!!!!
...
## Algorithm
Section explaining:
distinguish between input (the part that does not change during the whole calculations), calculation (the loop over time with updates of w) and the output section (operations on the final w result to calculate T for example). 
...
ADD MORE REFERENCES and references in text!!!!!!!!!!